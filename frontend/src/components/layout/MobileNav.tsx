...
export const MobileNav = () => {
  ...
  const mobileItems = user?.role === 'Student'
    ? items.filter((item) => ['Dashboard', 'Job Board', 'Skill Verification', 'Career Roadmap', 'Contracts', 'Wallet'].includes(item.label))
    : items.slice(0, 4);
  ...
  return (
    <nav
      ...
      aria-label="Primary"
    >
      <div className={`grid gap-1 py-1.5 ${mobileItems.length === 6 ? 'grid-cols-6' : mobileItems.length === 5 ? 'grid-cols-5' : 'grid-cols-4'}`}>
