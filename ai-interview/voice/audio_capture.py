...
# Set to True to print RMS values while listening (helps tune MIC_THRESHOLD)
_DEBUG_RMS = os.getenv("MIC_DEBUG", "").lower() in ("1", "true", "yes")

_ENERGY_THRESHOLD = float(os.getenv("MIC_THRESHOLD", "200.0"))
...
def _is_speech_energy(chunk: bytes) -> bool:
    ...
    if _DEBUG_RMS:
        ...
        print(f"\r  RMS:{rms:6.0f} |{bar:<30}| threshold={_ENERGY_THRESHOLD:.0f}", end="", flush=True)
...
class AudioCapture:
    ...
    def record_utterance(self, stop_event: threading.Event | None = None) -> bytes | None:
        ...
        """
        device_index = os.getenv("MIC_DEVICE_INDEX", "").strip()
        device_index_int = int(device_index) if device_index.isdigit() else None
        try:
            stream = self._pa.open(
                ...
                input_device_index=device_index_int,
            )
        except Exception as exc:
            print(f"Audio input open failed: {exc}")
            ...
            try:
                ...
                pass
            print("Set MIC_DEVICE_INDEX to one of the indexes above, e.g.")
            print('  setx MIC_DEVICE_INDEX "1"')
            return None
    ...
    def _is_speech(self, chunk: bytes) -> bool:
        # Force energy-based if MIC_FORCE_ENERGY=1
        if os.getenv("MIC_FORCE_ENERGY", "").lower() in ("1", "true", "yes"):
